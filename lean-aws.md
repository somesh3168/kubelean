...
master
ssh -i kubelean01.pem ubuntu@13.

worker-1

ssh -i kubelean01.pem ubuntu@13.169
...
# Apply restart sysctl params without reboot
sudo sysctl --system
```
...
1. **Initialize Kubernetes**:
   ...
   sudo kubeadm init --apiserver-advertise-address=172.31.33.84 --pod-network-cidr=192.168.0.0/16 --ignore-preflight-errors=Mem
   aws
   sudo kubeadm init --pod-network-cidr=10.244.0.0/16    aws
...
`kubectl expose deployment dep-name --type=LoadBalancer --name=my-service --external-ip=1.1.1.1`

--ignore-preflight-errors all 172.31.33.25:6443
kubeadm join --ignore-preflight-errors all 172.31.33.25:6443 --token fao2xs.f3sre3fegihnshbm --discovery-token-ca-cert-hash sha256:110a234d8ef0c414fbbd212b5c819f41d0745c0f26684ba7ec8eada66329a93
...
kubeadm join 172.31.33.25:6443 --token fao2xs.f3sre3fegihnshbm \
 --discovery-token-ca-cert-hash sha256:110a234d8ef0c414fbbd212b5c819f41d0745c0f26684ba7ec8eada66329a93
...
3. **On the Master server only, apply a common networking plugin. In this case, Flannel**:
   ...
   ```
   [option] kubectl apply -f https://raw.githubusercontent.com/coreos/flannel/master/Documentation/kube-flannel.yml
...
1.  **Create a Deployment**:
    ...
    ```
    or
2.  **Deploy the Nginx Deployment with 2 Pods** - Create an Nginx deployment with 2 replicas:
...
6.  **Get the NodePort and Access Nginx**:
    ```
